...
const STATE_FILE_PATH = path_1.default.join(__dirname, '../queue-state.json');
class QueueManager {
    state;
    tokenCounter = 0;
    actionLock = false;
    ...
    saveState() {
        try {
            ...
            fs_1.default.writeFileSync(STATE_FILE_PATH, data, 'utf8');
        }
        catch (error) {
    ...
    loadState() {
        try {
            if (fs_1.default.existsSync(STATE_FILE_PATH)) {
                const fileContent = fs_1.default.readFileSync(STATE_FILE_PATH, 'utf8');
    ...
    addPatient(name, symptoms, priority) {
        // Basic validation
        if (!name.trim())
            return null;
        // Handle duplicate patient names (edge case) by appending sequential count
        let displayName = name.trim();
        const countDuplicates = this.state.waiting.filter(p => p.name.toLowerCase().startsWith(displayName.toLowerCase())).length + (this.state.active?.name.toLowerCase().startsWith(displayName.toLowerCase()) ? 1 : 0);
    ...
    updateConfig(averageConsultationTime) {
    ...
    recalculateMetadataAndInsights() {
